-- drop old database
drop schema if exists `climbing_competition`;

-- create new schema
create schema if not exists `climbing_competition`
default collate 'utf8_general_ci' default character set 'utf8';

-- select database for use
use `climbing_competition`;

create table `site_table` (
	`id` integer primary key auto_increment,
	`table` varchar(64) unique not null,
	`description` varchar(128) default null
) engine = InnoDb comment 'monitoring tables list';
...
insert into `site_table` (`table`, `description`) values
('site_user','Таблца судей. К судьям относятся зарегистрированные пользователи сайта');

create table `site_userlog` (
	`id` integer(64) unsigned primary key,
	`uid` integer not null,
	`actiontime` timestamp not null default CURRENT_TIMESTAMP,
	`actiontype` enum  ('login',
		'logout',
		'preaction process',
		...
		'add data',
		'edit data')
	not null comment 'action decription',
	`table` integer not null comment 'table id',
	`record` integer default null comment 'changed record id',
	key `fk_userlog_uid` (`uid`),
	key `fk_userlog_table` (`table`) comment 'pointer to changed table description',
	constraint `fk_userlog_uid` foreign key (`uid`) references `site_user` (`uid`) on update cascade on delete restrict,
	constraint `fk_userlog_table` foreign key (`table`) references `site_table` (`id`) on update cascade on delete restrict
) engine = InnoDb comment 'user action log';
...
insert into `site_table` (`table`, `description`) values
('lib_climbdifficalty','Категории сложности трасс');
...
insert into `site_table` (`table`, `description`) values
('lib_sportrange','Спортивные разряды и звания');
...
insert into `site_table` (`table`, `description`) values
('lib_agegroup','Возрастные группы');
...
insert into `site_table` (`table`, `description`) values
('lib_competitiontype','Типы соревнований по скалолазанию');
...
insert into `site_table` (`table`, `description`) values
('lib_competitior','Таблица спортсменов');
...
insert into `site_table` (`table`, `description`) values
('competition','Описание соревнований');
...
insert into `site_table` (`table`, `description`) values
('round','Раунды (туры) соревнований.');
...
insert into `site_table` (`table`, `description`) values
('competition_member','Участники соревнований.');
...
insert into `site_table` (`table`, `description`) values
('route','Трассы для соревнований (с привязкой к туру)');
...
insert into `site_table` (`table`, `description`) values
('route_point','Нумерация зацепов для трасс');
...
insert into `site_table` (`table`, `description`) values
('preaction','Каталог электронных заявок');
...
create table `preaction_member` (
	...
	constraint `uk_preaction_member` unique (`round`, `uid`) comment 'unique member registration',
...
insert into `site_table` (`table`, `description`) values
('preaction_member','Участники электронных заявок');
